...
static inline 
void lock_init(lock_t *exclusion)
{
    *exclusion = 0;
...
#define lock_init(sl) ({*sl=0; __sync_synchronize();})

#define lock_rlock(sl) ({do{__loc_val = ACCESS_ONCE(*sl);read_read_barrier();}while((__loc_val & 0x1)); read_read_barrier(); })

#define lock_runlock(sl) ({ read_read_barrier(); long __sqtmp = ACCESS_ONCE(*sl); if((__loc_val != __sqtmp)) goto search_retry;})

#undef lock_validate
#define lock_validate(sl) lock_runlock(sl)

//#define lock_runlock(sl) ({if(__loc_val != ACCESS_ONCE(*sl)) goto search_retry;})

#define lock_wlock(sl)	({read_read_barrier(); *sl += 1; write_write_barrier(); })
#ifdef ONE_WRITER
#ifdef NO_WR_BR
#define lock_wunlock(sl) ({read_read_barrier(); *sl += 1; write_read_barrier(); })
#else
#define lock_wunlock(sl) ({read_read_barrier(); *sl += 1; write_write_barrier(); })
#endif


#else
#define lock_wlock(sl)	int __sqsucc = 0;			\
			long __sqold;				\
			do{					\
			__sqold = ACCESS_ONCE(*sl);		\
			if(__sqold & 0x1){continue;}		\
...
typedef volatile unsigned long lock_t __attribute__ ((__aligned__ (CACHE_LINE_SIZE)));
//typedef volatile unsigned long lock_t ;
#define lock_init(sl) ({*sl=0; __sync_synchronize();})

#ifdef ONE_WRITER

#define lock_rlock(sl) ({do{__loc_val = ACCESS_ONCE(*sl); read_read_barrier();}while(0); read_read_barrier();})

#define lock_runlock(sl) ({ read_read_barrier(); long __sqtmp = ACCESS_ONCE(*sl); if((__loc_val != __sqtmp)) goto search_retry;})
//#define lock_runlock(sl) ({ memory_barrier(); if(__loc_val != ACCESS_ONCE(*sl)) goto search_retry;})
...
#ifdef NO_WR_BR
#define lock_wunlock(sl) ({ *sl+= 1; write_write_barrier();}) 
#else
#define lock_wunlock(sl) ({ *sl+= 1; write_read_barrier();}) 
#endif
...
#define lock_runlock(sl) ({ read_read_barrier(); long __sqtmp = (ACCESS_ONCE(*sl) >> 1); if((__loc_val != __sqtmp)) goto search_retry;})
//#define lock_runlock(sl) ({ memory_barrier(); if(__loc_val != ACCESS_ONCE(*sl)) goto search_retry;})
...
#define lock_wlock(sl)	int __sqsucc = 0;			\
			...
			if(__oldv & 0x1){continue;}		\
			__sqsucc = __sync_bool_compare_and_swap(sl, __oldv, (__oldv | 0x1)); \
			}while(!__sqsucc);			\
			assert(ACCESS_ONCE(*sl) & 0x1)

#define lock_wunlock(sl) ({ __sync_fetch_and_add(sl, 1);}) //adding a 2(1 and 1 when we locked) will increment the GC(bits 31 to 1) and put to zero the lock(the bit 0)
...
#else
#define lock_wunlock(a) write_read_barrier(); //must flush the fifo, so the effect is visible within the function
#endif
