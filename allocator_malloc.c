...
void fnd(void* n){ free_node((node_t*)n); }
...
node_t* get_free()
{
	if(!lfl) {
		while(lfl = ACCESS_ONCE(freelist)) {
			if(__sync_bool_compare_and_swap(&freelist, lfl, NULL)) break;
		}
		if(!lfl) return NULL;
	}
	node_t* node = lfl;
...
node_t* get_free()
{
	...
	while(node && (!__sync_bool_compare_and_swap(&freelist, node, node->next)))
	{
		compiler_barrier();
		node = freelist;
		compiler_barrier();
...
void add_free(node_t* p)
{
	...
	if((lcnt >= AMORT_FREE_ON) && (ACCESS_ONCE(freelist) == NULL))
	{
		...
		if(__sync_bool_compare_and_swap(&freelist, NULL, lfl))//only one freer
		{
			write_write_barrier();//?
			lfl = NULL;
			lcnt = 0;
		...
		{
			if((__sync_val_compare_and_swap(&freelist, NULL, lfl) == NULL))//sucess
			{
				lfl=NULL;
				lcnt = 0;
				break;
...
void add_free(node_t* p)
{
	...
	if((lcnt >= AMORT_FREE_ON))
	{
		...
		compiler_barrier();
		//printf("%s %d| tofreelist %p, tail %p , mr_next %p\n", __FUNCTION__, thread, freelist, lfl_tail, lfl_tail->mr_next );
		compiler_barrier();
		while(!__sync_bool_compare_and_swap(&freelist, lfl_tail->mr_next, lfl))//only one freer
		{
...
{
		p->mr_next = freelist;
		while(!__sync_bool_compare_and_swap(&freelist, p->mr_next, p))//only one freer
		{
			p->mr_next = freelist;
		}
}
