...
void free_node_cb(struct rcu_head *head)
{
	...
	if (amort >= AMORT_COUNT) {
		...
		atomic_xadd4(&qsblockeds, (-AMORT_COUNT));// could steel get negatif? do a cas
		amort = 0;
	}
#else
	qsblockeds--;
...
{ 
	...
	amort++;
	if  (amort >= AMORT_COUNT) amort = 0;
	else if (amort == 1) atomic_xadd4(&qsblockeds, (AMORT_COUNT));	
#else
	qsblockeds++;
#endif
	//printf("%p after call rcu %p %ld\n", pthread_self(), node, qsblockeds);
#endif
}
